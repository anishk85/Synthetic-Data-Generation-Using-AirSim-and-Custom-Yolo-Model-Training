...
I collected data in two sessions:
\begin{itemize}
	\item Session 1 (\texttt{session\_20260213\_011051}): 19 images
	\item Session 2 (\texttt{session\_20260213\_011601}): 299 images
	\item \textbf{Total: 318 images} with corresponding label files
\end{itemize}

However, I noticed that in my environment, only Cars were visible and detected. The segmentation didn't find any Houses, Trees, or Roads - probably because of how the environment was set up or the specific area I flew in. This is something I learned - you need to fly in areas where all your target objects actually exist!
...
Class Distribution:
  Class 0 (House): 0 (0.0%)
  Class 1 (Tree): 0 (0.0%)
  Class 2 (Car): 896 (100.0%)
  Class 3 (Road): 0 (0.0%)

Total objects: 896
\end{verbatim}

As you can see, I only detected Cars in my dataset. This would affect the model performance for other classes.
...
These metrics are quite low, which makes sense because:
\begin{itemize}
	\item Small dataset (only 318 images total, 222 for training)
	\item Only one class had data (Cars)
	\item Some corrupted labels were excluded
	\item Limited variety in viewpoints/scenarios
\end{itemize}
...
\textbf{Test Results:}
\begin{verbatim}
Using model: runs/detect/runs/train/airsim_yolo_20260213_013915/weights/best.pt

Found 33 images to test

Processing: 100%
Detected 8 cars across all test images
Average confidence: 0.42

Results saved to: test_results/
\end{verbatim}

The test created 33 result images with bounding boxes drawn on detected cars. I could see:
\begin{itemize}
...
\textbf{Collection sessions:}
\begin{itemize}
	\item Session 1: 19 images, 19 labels
	\item Session 2: 299 images, 299 labels
	\item Total: 318 image-label pairs
\end{itemize}

\textbf{Data split:}
\begin{itemize}
	\item Training: 222 images (70\%)
	\item Validation: 63 images (20\%)
	\item Test: 33 images (10\%)
\end{itemize}

\textbf{Objects detected:}
\begin{itemize}
	\item Cars: 896 instances
	\item Houses: 0 instances
	\item Trees: 0 instances
	\item Roads: 0 instances
\end{itemize}
...
\begin{itemize}
	...
	\item Training time: ~15-20 minutes for 100 epochs
\end{itemize}

\textbf{Metrics on test set:}
\begin{itemize}
	\item mAP@50: 8.54\%
	\item mAP@50-95: 2.85\%
	\item Precision: 25.16\%
	\item Recall: 12.41\%
\end{itemize}

\textbf{Why metrics are low:}
\begin{enumerate}
	\item Small dataset (318 images is much less than the recommended 1000+)
	\item Only one class had training data (Cars only)
	\item Some labels were corrupted (out of bounds coordinates)
	\item Limited viewpoint diversity
	\item May need more training epochs or different hyperparameters
\end{enumerate}
...
\begin{figure}[h]
	...
	\begin{minipage}[t]{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{flightLogs/session_20260213_011601/images/00050.png}
		\caption*{(a) Raw RGB Camera Image}
	\end{minipage}\hfill
	\begin{minipage}[t]{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{test_results/result_test_00005.png}
		\caption*{(b) Model Detection Results}
	\end{minipage}
	\caption{Example of collected image and trained model prediction}
	\label{fig:lab6-example}
\end{figure}

\begin{figure}[h]
	\centering
	\includegraphics[width=0.85\textwidth]{runs/detect/runs/train/airsim_yolo_20260213_013915/results.png}
	\caption{Training curves showing loss and metrics over 100 epochs}
	\label{fig:lab6-training}
\end{figure}

\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\textwidth]{runs/detect/runs/train/airsim_yolo_20260213_013915/confusion_matrix.png}
...
\begin{figure}[h]
	...
	\begin{minipage}[t]{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{runs/detect/runs/train/airsim_yolo_20260213_013915/val_batch0_labels.png}
		\caption*{(a) Ground Truth Labels}
	\end{minipage}\hfill
	\begin{minipage}[t]{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{runs/detect/runs/train/airsim_yolo_20260213_013915/val_batch0_pred.png}
...
\begin{itemize}
	...
	\item Configured semantic segmentation for 4 target classes
	...
	\item Collected 318 labeled images in two flight sessions
...
\begin{itemize}
	...
	\item \textbf{Low performance:} With only 318 images and one class, the model didn't perform very well (8.5\% mAP). Would need much more data.
...
The main achievement was proving that synthetic data can be used to train object detection models. Getting 318 labeled images in less than 10 minutes of flight time shows how much faster this is compared to manually labeling photos.

However, the results also showed the limitations. My model only achieved 8.5\% mAP because:
\begin{itemize}
	\item Dataset was too small (318 images, should be 1000+)
	\item Only one class had training examples (Cars)
...
\begin{enumerate}
	\item Collect much more data (1000+ images minimum)
	\item Fly in diverse areas to get all object classes
